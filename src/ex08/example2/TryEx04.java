...
class Repository2 {
    void insert(String id, String pw) throws  RuntimeException{
        ...
        if (id.length() < 4) {
            throw new RuntimeException("id의 길이가 4글자 이상이어야 합니다");
        }
        if (pw.length() > 50) {
            throw new RuntimeException("pw의 길이가 50글자 이하이어야 합니다");
        }
    }
}
...
class Controller2 {
    void join(String id, String pw) throws  RuntimeException{
        ...
        if (id.length() < 4) {
            throw new RuntimeException("Controller: id의 길이가 4글자 이상이어야 합니다");
        }
